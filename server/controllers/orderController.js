...
exports.uploadBuyerFiles = async (req, res) => {
  try {
    ...
    if (!file_urls || !Array.isArray(file_urls) || file_urls.length === 0) {
      return res.status(400).json({
        success: false,
        error: "Please provide file URLs",
      });
    }
...
exports.deliverOrder = async (req, res) => {
  try {
    ...
    if (!file_urls || !Array.isArray(file_urls) || file_urls.length === 0) {
      return res.status(400).json({
        success: false,
        error: "Please provide delivery file URLs",
      });
    }
...
exports.completeOrder = async (req, res) => {
  try {
    ...
    // If escrow was held, it should be released here
    // This will be handled by the payment controller's releaseEscrow function
    if (order.escrow_status === "held") {
      // TODO: Call payment controller to release escrow
